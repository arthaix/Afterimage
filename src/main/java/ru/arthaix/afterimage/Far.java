 ...
 * the far clipping plane pushed out and fog off, then the depth buffer is cleared, so vanilla terrain (always
 * nearer) draws on top. A copy is dropped as soon as its chunk is loaded again and vanilla has compiled it.
...
public final class Far {
        ...
        try {
            ...
            if (chunkLoaded(p)) {
                if (next != CompiledChunk.field_178502_a) {
                    drop(p.func_177986_g());
                    Disk.onVanillaCompiled(p.func_177986_g(), next);
                }
            } else {
                capture(rc);
            }
    ...
    private static void renderImpl(double pt, Entity e, ViewFrustum frustum) {
        ...
        PROJ[10] = (float) (-(newFar + near) / (newFar - near));
        PROJ[14] = (float) (-2.0 * newFar * near / (newFar - near));
    ...
    public static String summary() {
        return "far " + (ENABLED ? "ON" : "OFF") + ": sections " + ENTRIES.size() + ", VRAM "
            + String.format("%.1f MB", bytes / 1048576.0) + " of " + (BUDGET >> 20) + " MB, captures " + captures
